...
ZARBS <- function(mu.link = "log", sigma.link = "log", nu.link = "logit")
{
  ...
  structure(
                 ...
                 rqres <- expression(rqres(pfun = "pZARBS",type = "Mixed", mass.p = 0, prob.mp = nu,y = y, mu = mu, sigma = sigma, nu = nu)),
                 mu.initial <- expression({mu <- rep(mean(y[y>0]),length(y))}),
                 sigma.initial <- expression({sigma <- rep(sigmatil(y[y>0]),length(y))}),
                 nu.initial <- expression({nu <- rep(length(y[y==0])/length(y),length(y))}),
...
#'   return(integrate(E,0,1.0e120)$value)
#' }
